...
var templates = {
    ...
    init: function (config, cb) {
        ...
        if (!config || !config.dir) {
            throw new Error('Template config dir not set: ' + JSON.stringify(
                config));
        }
        ...
        function loadedTemplates(err, files) {
            if (err || !files) {
                return cb(err || new Error('no template loaded! config: ' +
                    JSON.stringify(config)));
            }

            files.forEach(function (file) {
                try {
                    file.subject = jade.compile(file.subject);
                    file.body = jade.compile(file.body);

                    templates.map[file.name || file.filename.split('.').shift()] =
                        file;
                } catch (e) {
                    // continue loading other templates
                    util.log('error loading template: ' + file.name + ' ' +
                        e.message);
                }
            });
